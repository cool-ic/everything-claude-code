...
| 任务类型                 | 模型   | 原因                                       |
| ------------------------- | ------ | ------------------------------------------ |
| 探索/搜索                | qwen3.5-plus  | 快速、便宜，足以用于查找文件               |
| 简单编辑                 | qwen3.5-plus  | 单文件更改，指令清晰                       |
| 多文件实现               | qwen3.5-plus | 编码的最佳平衡                             |
| 复杂架构                 | qwen3.5-plus   | 需要深度推理                               |
| PR 审查                  | qwen3.5-plus | 理解上下文，捕捉细微差别                   |
| 安全分析                 | qwen3.5-plus   | 不能错过漏洞                               |
| 编写文档                 | qwen3.5-plus  | 结构简单                                   |
| 调试复杂错误             | qwen3.5-plus   | 需要将整个系统记在脑中                     |

对于 90% 的编码任务，默认使用 qwen3.5-plus。当第一次尝试失败、任务涉及 5 个以上文件、架构决策或安全关键代码时，升级到 qwen3.5-plus。
